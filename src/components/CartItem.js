...
import { deleteItem, increamentQuantity, resetCart,decrementQuantity } from '../redux/proSlice';
const CartItem = () => {
  ...
  return (
    <div className='w-2/3 pr-10 mr-10'>
      <div className='w-full'>
      <h2 className='text-2xl text-left'>shopping cart</h2>
      <div>
      {
        productData.map((item)=>{
            return <div className='flex items-center justify-between gap-6 mt-6' key={item._id}>
                <div className='flex items-center gap-2'>
                <MdOutlineClose onClick={()=> dispatch(
                    deleteItem(item._id)
                ) & toast.error(`${item.title} is removed`)} className='text-xl text-gray-600 hover:text-red-600 cursor-pointer duration-300'/>
                <img className='w-32 h-32 object-cover' src={item.image} alt='productImg'/>
                </div>
                <h2 className='w-52 text-left'>{item.title}</h2>
